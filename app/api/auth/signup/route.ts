...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      const userLocale = validatedData.language === 'fr' ? 'fr' : 'en';
      await createWelcomeMessage(user.id, userLocale);
      ...
      // Log but don't fail registration if welcome message fails
      console.error('Failed to create welcome message or send email:', messageError);
    }
    ...
    if (error instanceof ZodError) {
        ...
        // DEBUG
        console.error('Validation error during registration:', error.issues);
